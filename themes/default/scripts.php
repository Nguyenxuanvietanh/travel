 ...
 return base_url + return_hash;
  ...
  $(function(){
    ...
    var now = new Date(nowTemp.getFullYear(), nowTemp.getMonth(), nowTemp.getDate(), 0, 0, 0, 0);
        ...
        {
          placeholder: "<?php if(empty($query->destination)){ echo "Enter City Or Airport"; }else{ echo $query->origin; } ?>",
          minimumInputLength: 3,
        ...
        $('#location_to').select2(
        {
          placeholder: "<?php if(empty($query->origin)){ echo "Enter City Or Airport"; }else{ echo $query->destination ; } ?>",
          minimumInputLength: 3,
          width:'100%', maximumSelectionSize: 1,
          initSelection: function (element, callback) {
            var data = {id: "1", text: "<?php echo ""; ?>"};
