...
class ServiceController {
    ...
    public function __construct()
    {
        ...
        $this->public_key = $apiConfig->public_key; //"6e79tsxfyxxvp7qdkfac8kz3";
        $this->secret_key = $apiConfig->secret_key; //"PMQyprXPmD";
...
//        $service = $this->getServiceUri($payload);
        ...
        {
            if($this->fakeresponse == 'success') {
                $availability_resp = __DIR__ . '/Responses/'.$service.'_response_success'.'.json';
            } else {
                $availability_resp = __DIR__ . '/Responses/'.$service.'_response_fail'.'.json';
            }
        ...
        {	
            ...
            if($service == 'status') {
                curl_setopt_array($curl, array(
                    CURLOPT_RETURNTRANSFER => 1,
                    CURLOPT_URL => $this->hotel_api . $service,
                    CURLOPT_HTTPHEADER => [
                        'Accept:application/json', 
                        'Api-key:'.$this->public_key, 
                        'X-Signature:'.$this->getSignature()
