...
class Hotelbeds extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->data['concepts'] = $this->dbhb->select('*')->where_in('description', ['Bed and breakfast','Half Board','Full board','All inclusive'])->get('facilities')->result();
    ...
    {
        try {
            $this->data['detailpage_uri'] = $_SESSION['detailpage_uri'];
...
//        print_r($this->data['concepts']);
        ...
        if(in_array('filter', $args))
        {
            list($country,$city,$checkIn,$checkOut,$adults,$childs,$filter,$stars,$price,$facilities,$accommodation) = $args;
            list($minRate, $maxRate) = explode(',', $price);
            $filter = new StdClass();
            $review = new StdClass();
            $keyword = new StdClass();
            ...
            $availabilityReq->filter = $filter;
            $_keywords = explode('-', $facilities);
            $keyword->keyword = array();
            foreach ($_keywords as $i => $key) {
                $key = explode(":", $key)[2];
                array_push($keyword->keyword, $key);
            }
            $availabilityReq->keywords = $keyword;
...
//        die();
        ...
        $this->data['uri'] = 'hotelb/search/'.$country.'/'.$city.'/'.$checkIn.'/'.$checkOut.'/'.$adults.'/'.$childs;
        $this->data['detailpage_uri'] = 'hotelb/detail/%s/'.$checkIn.'/'.$checkOut.'/'.$adults.'/'.$childs;
    ...
    public function detail(...$args)
    {
        ...
        if(count($args) > 5) {
            list($recheck,$hotelname,$checkIn,$checkOut,$adults,$childs) = $args;
        }
        $this->data['loadMap'] = true;
        $hotelname = str_replace(["%20", "-"],[" "," "], $hotelname);
        $this->dbhb->where('LOWER(name)', $hotelname);
        $this->data['hotel'] = $this->dbhb->get('hotels')->row();
        if(count($args) > 5)
        {
            $checkin = explode('-', $checkIn);
            $checkout = explode('-', $checkOut);
        ...
        $this->data['featuredHotelsUrl'] = base_url("hotelb/detail/%s/{$checkIn}/{$checkOut}/{$adults}/{$childs}");
    ...
    public function booking()
    {
        try {
            ...
            $this->Booking->hotel_location = 'NULL';
            ...
            $this->output->set_output(json_encode([
                'status' => 'fail',
                'message' => $e->getMessage(),
                'invoice_url' => $_SESSION['detailPageUrl']
            ]));
        }
    }
